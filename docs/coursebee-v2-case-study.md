...
CourseBee now separates low-level factual retrieval from high-level overview and relationship retrieval through `mode="auto"`. This is LightRAG-style routing, not a full LightRAG implementation.
...
CourseBee는 chunk-level retrieval만 하지 않고, chunk -> lecture -> course pack 수준의 hierarchical summary index를 구성해 local detail question과 global overview question을 분리 처리합니다.

이 방식은 RAPTOR-style 접근에서 아이디어를 가져오지만, 현재 구현은 임베딩 클러스터링 기반 tree가 아니라 강의자료의 자연스러운 구조를 활용하는 course-structure-aware hierarchical retrieval입니다.
...
CourseBee v2의 GraphRAG-lite는 full Microsoft GraphRAG가 아닙니다. 이 프로젝트에서는 Course Pack 내부의 Document, Lecture, Page, Chunk, Concept node와 evidence-backed relation edge를 retrieval에 활용하는 lightweight graph-augmented retrieval로 범위를 제한했습니다.
...
| Retrieval | Single-document RAG | Auto-routed chunk, hierarchical summary, and course graph retrieval |
